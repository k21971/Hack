...
/*
 * Scroll reading system for 1984 Hack - magical scroll effects and
 * identification Original 1984 source:
 * docs/historical/original-source/hack.read.c
 *
...
int doread(void) {
  ...
  case SCR_ENCHANT_ARMOR: {
    ...
    if (confused) {
      pline("Your %s glows silver for a moment.", objects[otmp->otyp].oc_name);
      otmp->rustfree = 1;
      break;
    }
    ...
    pline("Your %s glows green for a moment.", objects[otmp->otyp].oc_name);
    otmp->cursed = 0;
    otmp->spe++;
    break;
  }
  case SCR_DESTROY_ARMOR:
    if (confused) {
      ...
      pline("Your %s glows purple for a moment.", objects[otmp->otyp].oc_name);
      otmp->rustfree = 0;
      break;
    }
    if (uarm) {
  ...
  case SCR_CREATE_MONSTER: {
    ...
    if (confused)
      cnt += 12;
    while (cnt--)
      (void)makemon(confused ? PM_ACID_BLOB : (struct permonst *)0, u.ux, u.uy);
    break;
  }
  case SCR_ENCHANT_WEAPON:
    if (uwep && confused) {
      pline("Your %s glows silver for a moment.", objects[uwep->otyp].oc_name);
      uwep->rustfree = 1;
  ...
  case SCR_DAMAGE_WEAPON:
    if (uwep && confused) {
      pline("Your %s glows purple for a moment.", objects[uwep->otyp].oc_name);
      uwep->rustfree = 0;
    } else if (!chwepon(scroll, -1)) /* tests for !uwep */
      return (1);
    break;
  ...
  case SCR_IDENTIFY:
    ...
    useup(scroll);
    SAFE_OBJECTS(SCR_IDENTIFY).oc_name_known =
        1; /* MODERN: Safe array access */
    if (!confused)
...
int identify(struct obj *otmp) /* also called by newmail() */
{
...
int monstersym(char ch) /* arnold@ucsfcgl */
{
  struct permonst *mp;
