...
int hitmm(struct monst *magr, struct monst *mdef) {
  ...
  if (vis) {
    ...
    if (magr->mimic)
      seemimic(magr);
    (void)snprintf(buf, BUFSZ, "%s %s",
                   Monnam(magr), /* MODERN: Safe sprintf replacement - identical
...
boolean hmon(struct monst *mon, struct obj *obj,
             int thrown) /* return TRUE if mon still alive */
{
  int tmp;
  ...
  if (!hittxt) {
    if (thrown)
      ...
      hit(xname(obj) /* or: objects[obj->otyp].oc_name */, mon, exclam(tmp));
    else if (Blind)
      pline("You hit it.");
    else
...
int attack(struct monst *mtmp) {
  ...
  if (mdat->mlet == 'L' && !mtmp->mfroz && !mtmp->msleep && !mtmp->mconf &&
      mtmp->mcansee && !rn2(7) &&
  ...
  if (uwep) {
    ...
    if (uwep->otyp == TWO_HANDED_SWORD)
      ...
      tmp += 3;
    else if (uwep->otyp == SPEAR && index("XDne", mdat->mlet))
      tmp += 2;
  }
  if (mtmp->msleep) {
    mtmp->msleep = 0;
  ...
  if (malive && mdat->mlet == 'E' && canseemon(mtmp) && !mtmp->mcan && rn2(3)) {
    if (mtmp->mcansee) {
