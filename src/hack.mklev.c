...
extern unsigned char xdnstair, xupstair, ydnstair,
    yupstair; /* MODERN: unsigned to prevent buffer underflow */
...
int addrs(int lowx, int lowy, int hix, int hiy) {
  struct rectangle *rsp;
...
int addrsx(int lx, int ly, int hx, int hy,
...
int comp(const void *x, const void *y) {
  const struct mkroom *a = (const struct mkroom *)x;
  const struct mkroom *b = (const struct mkroom *)y;
  if (a->lx < b->lx)
    return (-1);
  return (a->lx > b->lx);
}
...
void dodoor(int x, int y, struct mkroom *aroom) {
  if (doorindex >= DOORMAX) {
    impossible("DOORMAX exceeded?", 0, 0);
    return;
  }
  if (!okdoor(x, y) && nxcor)
    return;
...
void makevtele() { makeniche(TRUE); }

void makeniche(boolean with_trap) {
  struct mkroom *aroom;
  struct rm *rm;
  int vct = 8;
  coord dd;
