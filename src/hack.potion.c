...
int dodrink(void) {
  ...
  case POT_LEVITATION:
    ...
    break;
  default:
    impossible("What a funny potion!", otmp->otyp, 0);
    return (0);
  }
  if (nothing) {
    unkn++;
...
/* MODERN: CONST-CORRECTNESS: bottlenames is a read-only array of string
 * literals */
...
void potionhit(struct monst *mon, struct obj *obj) {
  ...
  if (isyou) {
    uclose = TRUE;
    pline("The %s crashes on your head and breaks into shivers.", botlnam);
    losehp(rnd(2), "thrown potion");
  } else {
    uclose = (dist(mon->mx, mon->my) < 3);
    /* perhaps 'E' and 'a' have no head? */
    pline("The %s crashes on %s's head and breaks into shivers.", botlnam,
          monnam(mon));
    if (rn2(5) && mon->mhp > 1)
      mon->mhp--;
  }
...
int dodip(void) {
  ...
  if (!(potion = getobj("!", "dip into")))
    return (0);
  pline("Interesting...");
  if (obj->otyp == ARROW || obj->otyp == DART || obj->otyp == CROSSBOW_BOLT) {
