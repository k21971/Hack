...
int mhitu(struct monst *mtmp) {
  ...
  if (u.uswallow) {
    if (mtmp != u.ustuck) {
      if (mdat->mlet == 'c' && !rn2(13)) {
        pline("Outside, you hear %s's hissing!", monnam(mtmp));
        pline("%s gets turned to stone!", Monnam(u.ustuck));
    ...
    case ',':
      youswld(mtmp, (u.uac > 0) ? u.uac + 4 : 4, 5, "The trapper");
      break;
    case '\'':
  ...
  case 'c':
    if (!rn2(5)) {
      ...
      if (ctmp || !rn2(20) ||
          (flags.moonphase == NEW_MOON && !carrying(DEAD_LIZARD))) {
        Stoned = 5;
  ...
  case 'x': {
    ...
    pline("%s pricks in your %s leg!", Monnam(mtmp),
