...
/* extern struct obj *mkobj_at(); - declared below with proper signature */

/* Forward declarations for functions not yet prototyped in hack.h */
...
/* wipe_engr_at() and sengr_at() now declared in hack.h */
...
extern int hitmm(struct monst *mtmp1, struct monst *mtmp2);
extern int hitu(struct monst *mtmp, int d);
...
extern void replshk(struct monst *old, struct monst *new);
extern void replgd(struct monst *old, struct monst *new);
extern long newuexp(void);
extern void more_experienced(int exp, int rexp);
/* mksobj_at already declared in hack.h with correct signature */
extern void stackobj(struct obj *obj);
extern struct obj *mkobj_at(int let, int x, int y);
/* atl already declared in hack.h with correct signature */
...
extern void losestr(int n);
extern void relobj(struct monst *mtmp, int flag);

int warnlevel; /* used by movemon and dochugw */
long lastwarntime;
int lastwarnlev;
/* MODERN: CONST-CORRECTNESS: warning color strings are read-only */
const char *const warnings[] = {"white", "pink",   "red",
                                "ruby",  "purple", "black"};
...
void movemon(void) {
  ...
  next_mon:
    ...
    if (mtmp->mspeed != MSLOW || !(moves % 2)) {
      ...
      fr = -1;
      if (Conflict && cansee(mtmp->mx, mtmp->my) && (fr = fightm(mtmp)) == 2)
        continue;
...
int dochug(struct monst *mtmp) {
  ...
  int tmp = 0, nearby = 0,
      scared = 0; /* MODERN: Initialize to prevent Valgrind warnings */
  ...
  /* regenerate monsters */
  if ((!(moves % 20) || index(MREGEN, mdat->mlet)) && mtmp->mhp < mtmp->mhpmax)
    mtmp->mhp++;
  ...
  if (!nearby || mtmp->mflee || mtmp->mconf || (mtmp->minvis && !rn2(3)) ||
  ...
  if (!index("Ea", mdat->mlet) && nearby && !mtmp->mpeaceful && u.uhp > 0 &&
      !scared) {
    if (mhitu(mtmp))
...
int m_move(struct monst *mtmp, int after) {
  ...
  schar chi;
  boolean likegold = FALSE, likegems = FALSE,
          likeobjs =
              FALSE; /* MODERN: Initialize to prevent uninitialized use */
  ...
  if (msym == 'U' && !mtmp->mcan && canseemon(mtmp) && mtmp->mcansee &&
      rn2(5)) {
    if (!Confusion)
...
not_special:
  ...
  if (mtmp->mflee)
    appr = -1;
  if (mtmp->mconf || Invis || !mtmp->mcansee || (index("BIy", msym) && !rn2(3)))
    appr = 0;
  omx = mtmp->mx;
  omy = mtmp->my;
  gx = u.ux;
...
#endif /* STUPID */
    if ((appr == 1 && nearer) || (appr == -1 && !nearer) || !mmoved ||
        (!appr && !rn2(++chcnt))) {
      nix = nx;
      niy = ny;
      chi = i;
...
#define MAXMONNO 100
  ...
  if (tmp >= 0 && tmp < CMNUM + 2) {
    ...
    u.nr_killed[tmp]++;
    if ((nk = u.nr_killed[tmp]) > MAXMONNO && !index(fut_geno, mdat->mlet))
...
#endif /* NOWORM */
    if (!letter(tmp) || (!index("mw", tmp) && !rn2(3)))
      tmp = 0;
...
void rescham(void) /* force all chameleons to become normal */
{
  struct monst *mtmp;
...
int newcham(
    struct monst *mtmp,
    struct permonst *mdat) /* make a chameleon look like a new monster */
...
int ishuman(struct monst *mtmp) { return (mtmp->data->mlet == '@'); }
