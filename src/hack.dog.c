...
struct permonst li_dog = {"little dog",       'd', 2, 18, 6, 1, 6,
                          sizeof(struct edog)};
struct permonst dog = {"dog", 'd', 4, 16, 5, 1, 6, sizeof(struct edog)};
struct permonst la_dog = {"large dog", 'd', 6, 15,
                          4,           2,   4, sizeof(struct edog)};
...
int dogfood(struct obj *obj) {
  switch (obj->olet) {
  case FOOD_SYM:
    return ((obj->otyp == TRIPE_RATION) ? DOGFOOD
...
int dog_move(struct monst *mtmp, int after) {
  ...
  int udist, chi = -1, i,
             whappr; /* MODERN: Initialize chi to prevent uninitialized use */
  struct monst *mtmp2;
  ...
  unsigned char gx = 0,
                gy = 0; /* MODERN: Initialize to prevent uninitialized use */
  xchar gtyp,
      otyp; /* current goal - gx,gy unsigned to prevent buffer underflow */
  coord poss[9];
  int info[9];
...
#define DDIST(x, y) ((x - omx) * (x - omx) + (y - omy) * (y - omy))
  ...
  if (mtmp->minvent) {
    if (!rn2(udist) || !rn2((int)edog->apport))
      if ((unsigned)rn2(10) <
          edog->apport) { /* MODERN: Cast to unsigned to match apport type */
    ...
    if ((obj = o_at(omx, omy)))
      if (!index("0_", obj->olet)) {
        ...
        if (obj->owt < 10 * mtmp->data->mlevel)
          if ((unsigned)rn2(20) <
              edog->apport +
                  3) /* MODERN: Cast to unsigned to match apport type */
            if (rn2(udist) || !rn2((int)edog->apport)) {
              freeobj(obj);
              unpobj(obj);
  ...
  for (obj = fobj; obj; obj = obj->nobj) {
    ...
    if (inroom(obj->ox, obj->oy) != dogroom)
      continue;
    if (otyp < MANFOOD && (dogroom >= 0 || DDIST(obj->ox, obj->oy) < 10)) {
      if (otyp < gtyp ||
          (otyp == gtyp && DDIST(obj->ox, obj->oy) < DDIST(gx, gy))) {
        gx = obj->ox;
        gy = obj->oy;
        gtyp = otyp;
      }
    } else if (gtyp == UNDEF && dogroom >= 0 && uroom == dogroom &&
               !mtmp->minvent &&
               edog->apport >
                   (unsigned)rn2(
                       8)) { /* MODERN: Cast to unsigned to match apport type */
      gx = obj->ox;
      gy = obj->oy;
      gtyp = APPORT;
    }
  ...
  if (gtyp == UNDEF ||
    ...
    if (dogroom < 0 || dogroom == uroom) {
      ...
      int tmp =
          rooms[(unsigned char)dogroom].fdoor; /* MODERN: Cast to unsigned char
                                                  for safe array indexing */
      cnt =
          rooms[(unsigned char)dogroom].doorct; /* MODERN: Cast to unsigned char
                                                   for safe array indexing */
    ...
    if (after && udist <= 4 && gx == u.ux && gy == u.uy)
      return (0);
    if (udist > 1) {
      if (!IS_ROOM(levl[(unsigned char)u.ux][(unsigned char)u.uy].typ) ||
          !rn2(4) || /* MODERN: Cast to unsigned char for safe array indexing */
          whappr || (mtmp->minvent && rn2((int)edog->apport)))
        appr = 1;
    }
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (info[i] & ALLOW_M) {
      mtmp2 = m_at(nx, ny);
      if (mtmp2->data->mlevel >= mdat->mlevel + 2 || mtmp2->data->mlet == 'c')
        continue;
      if (after)
        return (0); /* hit only once each move */

      if (hitmm(mtmp, mtmp2) == 1 && rn2(4) && mtmp2->mlstmv != moves &&
          hitmm(mtmp2, mtmp) == 2)
        return (2);
      return (0);
    }
    ...
    while (obj) {
      ...
      if (obj->cursed)
        goto nxti;
      if (obj->olet == FOOD_SYM && (otyp = dogfood(obj)) < MANFOOD &&
  ...
  *mtmp2 = *mtmp;
  ...
  if (mtmp->mnamelth)
    (void)strcpy(NAME(mtmp2),
                 NAME(mtmp)); /* MODERN: Safe - allocated exact size */
  initedog(mtmp2);
  replmon(mtmp, mtmp2);
  return (1);
}
