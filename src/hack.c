 ...
 */
void seeoff(int mode) /* 1 to redo @, 0 to leave them */
...
void domove(void) {
  ...
  if (u.uswallow) {
    ...
    if (Confusion) {
       ...
       * WHY: Original code accessed levl[u.ux+u.dx][u.uy+u.dy] before isok()
       * bounds check HOW: Store coordinates in variables, check bounds before
       * array access PRESERVES: Original 1984 confusion logic exactly ADDS:
       * Memory safety by preventing out-of-bounds array access
       */
      int newx, newy;
      do {
        confdir();
        newx = u.ux + u.dx;
        newy = u.uy + u.dy;
      } while (!isok(newx, newy) || IS_ROCK(levl[newx][newy].typ));
    }
  ...
  u.uy += u.dy;
  if (flags.run) {
    if (tmpr->typ == DOOR || (xupstair == u.ux && yupstair == u.uy) ||
...
int pickup(int all) {
  ...
  for (obj = fobj; obj; obj = obj2) {
    ...
    if (obj->ox == u.ux && obj->oy == u.uy) {
      ...
      too_heavy:
        pline("There %s %s here, but %s.", (obj->quan == 1) ? "is" : "are",
              doname(obj),
    ...
    lift_some:
      ...
      addtobill(obj); /* sets obj->unpaid if necessary */
      {
        ...
        int mergquan;
        if (!Blind)
          obj->dknown = 1; /* this is done by prinv(),
...
void lookaround(void) {
  int x, y, i, x0 = 0, y0 = 0, m0 = 0,
               i0 = 9; /* MODERN: Initialize to prevent uninitialized use */
  ...
  for (x = u.ux - 1; x <= u.ux + 1; x++)
    for (y = u.uy - 1; y <= u.uy + 1; y++) {
      ...
      if ((mtmp = m_at(x, y)) && !mtmp->mimic &&
        ...
        mtmp = 0; /* invisible M cannot influence us */
...
int sgn(int a) { return ((a > 0) ? 1 : (a == 0) ? 0 : -1); }
...
void losestr(
    int num) /* may kill you; cause may be poison or monster like 'A' */
{
  u.ustr -= num;
  while (u.ustr < 3) {
    u.ustr++;
...
void losexp(void) /* hit by V or W */
{
  int num;
  extern long newuexp();
...
long newuexp(void) { return (10 * (1L << (u.ulevel - 1))); }
