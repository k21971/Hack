...
void off_msg(struct obj *otmp) { pline("You were wearing %s.", doname(otmp)); }
...
void glibr(void) {
  ...
  if ((otmp = uwep) != Null(obj)) {
    ...
    setuwep((struct obj *)0);
    dropx(otmp);
    pline("Your weapon %sslips from your hands.", xfl ? "also " : "");
  }
}
...
void corrode_armor(void) {
  ...
  if (otmph) {
    if (otmph->rustfree || otmph->otyp == ELVEN_CLOAK ||
        otmph->otyp == LEATHER_ARMOR || otmph->otyp == STUDDED_LEATHER_ARMOR) {
