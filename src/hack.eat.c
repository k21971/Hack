...
struct {
  const char *txt;
  int nut;
} tintxts[] = {/* MODERN: const because txt points to string literals */
    ...
    for (otmp = fobj; otmp; otmp = otmp->nobj) {
      if (otmp->ox == u.ux && otmp->oy == u.uy && otmp->olet == FOOD_SYM) {
        pline("There %s %s here; eat %s? [ny] ",
              (otmp->quan == 1) ? "is" : "are", doname(otmp),
              (otmp->quan == 1) ? "it" : "one");
        if (readchar() == 'y') {
          if (otmp->quan != 1)
...
eatx:
  if (multi < 0 && !nomovemsg) {
    ...
    (void)snprintf(
        msgbuf, BUFSZ,
        "You finished eating the %s.", /* MODERN: Safe sprintf replacement -
