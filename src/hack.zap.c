...
struct monst *bhit(int ddx, int ddy, int range, char sym, int (*fhitm)(), int (*fhito)(), struct obj *obj)
{					/* direction and range */
...
void fracture_rock(struct obj *obj)	/* fractured by pick-axe or wand of striking */
{					   /* no texts here! */
	/* unpobj(obj); */
	obj->otyp = ROCK;
	obj->quan = 7 + rn2(60);
