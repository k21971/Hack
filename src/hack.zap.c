...
static int goodpos(int x, int y) {
...
const char *const fl[] = {"magic missile", "bolt of fire", "sleep ray",
                          "bolt of cold", "death ray"};
...
int bhito(struct obj *obj,
          struct obj *otmp) /* object obj was hit by the effect of wand otmp */
{                           /* returns TRUE if sth was done */
  ...
  if (obj == uball || obj == uchain)
    ...
    case WAN_POLYMORPH:
      ...
      mkobj_at((obj->otyp == ROCK || obj->otyp == ENORMOUS_ROCK) ? GEM_SYM
                                                                 : obj->olet,
               obj->ox, obj->oy)
          ->quan = obj->quan;
      delobj(obj);
    ...
    case WAN_DIGGING:
       ...
       */
      {
        ...
        if (u.uswallow) {
          ...
          pline("You pierce %s's stomach wall!", monnam(mtmp));
...
struct monst *bhit(int ddx, int ddy, int range, char sym, int (*fhitm)(),
                   int (*fhito)(), struct obj *obj) { /* direction and range */
...
int zhit(struct monst *mon, int type) /* returns damage to mon */
{
  int tmp = 0;
  ...
  case 1:  /* fire */
    if (index("Dg", mon->data->mlet))
      break;
    tmp = d(6, 6);
    if (index("YF", mon->data->mlet))
      tmp += 7;
    break;
  case 2: /* sleep*/
    mon->mfroz = 1;
...
void fracture_rock(
    struct obj *obj) /* fractured by pick-axe or wand of striking */
{                    /* no texts here! */
  /* unpobj(obj); */
  obj->otyp = ROCK;
  obj->quan = 7 + rn2(60);
