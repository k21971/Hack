      ...
      for (otmp = invent; otmp; otmp = otmp->nobj)
        if (otmp->olet == AMULET_SYM && !otmp->spe) {
          ...
          if (dist(mtmp->mx, mtmp->my) > 2)
            pline("You get the creepy feeling that somebody noticed your "
                  "taking the Amulet.");
          return;
        }
  }
...
hithim:
  if (rn2(2)) { /* hit - perhaps steal */
    ...
    if (hitu(mtmp, d(mtmp->data->damn, mtmp->data->damd)) && !rn2(20) &&
        stealamulet(mtmp)) {
    }
  } else
...
void inrange(struct monst *mtmp) {
  ...
  if ((!tx && abs(ty) < BOLT_LIM) || (!ty && abs(tx) < BOLT_LIM) ||
    ...
    case '1':
      ...
      if (mtmp->mcan || rn2(2)) {
        if (canseemon(mtmp))
          pline("%s makes a gesture, then curses.", Monnam(mtmp));
        else
...
void clonewiz(struct monst *mtmp) {
  struct monst *mtmp2;
