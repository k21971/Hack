...
int dodrop(void) { return (drop(getobj("0$#", "drop"))); }
...
static int drop(struct obj *obj) {
  ...
  if (obj->olet == '$') { /* pseudo object */
    ...
    if (amount == 0)
      ...
      pline("You dropped %ld gold piece%s.", amount, plur(amount));
      if (Invisible)
        newsym(u.ux, u.uy);
    }
    free((char *)obj);
...
/* drop several things */
int doddrop(void) { return (ggetobj("drop", drop, 0)); }
...
void goto_level(int newlevel, boolean at_stairs) {
  ...
  if (fd < 0) {
     ...
     * writable.
     */
    pline("A mysterious force prevents you from going %s.", up ? "up" : "down");
    return;
  }
...
int donull(void) { return (1); /* Do nothing, but let other things happen */ }
...
int dothrow(void) {
  ...
  if (mon) {
    ...
    if (obj->olet == WEAPON_SYM) {
      ...
      if (obj->olet == GEM_SYM && mon->data->mlet == 'u' && !mon->mtame) {
        /* MODERN: Add bounds checking for objects array access */
        /* Note: otyp is uchar, so >= 0 check is redundant */
        if (obj->otyp < NROFOBJECTS && obj->dknown &&
  ...
  /* subfrombill(obj); */
  stackobj(obj);
  if (Punished && obj == uball && (bhitpos.x != u.ux || bhitpos.y != u.uy)) {
    freeobj(uchain);
    unpobj(uchain);
    if (u.utrap) {
