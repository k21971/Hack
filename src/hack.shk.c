...
void obfree(struct obj *obj, struct obj *merge) { free((char *)obj); }
int inshop(void) { return (0); }
void shopdig(void) {}
void addtobill(void) {}
void subfrombill(void) {}
void splitbill(void) {}
int dopay(void) { return (0); }
void paybill(void) {}
int doinvbill(void) { return (0); }
void shkdead(void) {}
int shkcatch(void) { return (0); }
int shk_move(void) { return (0); }
void replshk(struct monst *mtmp, struct monst *mtmp2) {}
char *shkname(void) { return (""); }
...
char shtypes[] = {/* 8 shoptypes: 7 specialized, 1 mixed */
                  RING_SYM,   WAND_SYM,   WEAPON_SYM, FOOD_SYM,
                  SCROLL_SYM, POTION_SYM, ARMOR_SYM,  0};

/* MODERN: CONST-CORRECTNESS: shop name strings are read-only */
static const char *const shopnam[] = {
    "engagement ring",  "walking cane", "antique weapon", "delicatessen",
    "second hand book", "liquor",       "used armor",     "assorted antiques"};

char *shkname(struct monst *mtmp) /* called in do_name.c */
{
...
void shkdead(struct monst *mtmp) /* called in mon.c */
{
  struct eshk *eshk = ESHK(mtmp);

  if (eshk->shoplevel == dlevel)
    rooms[(unsigned char)(unsigned char)eshk->shoproom].rtype =
        0; /* MODERN: Cast to unsigned char for safe array indexing */
  if (mtmp == shopkeeper) {
    setpaid();
    shopkeeper = 0;
...
static void setpaid(void) { /* caller has checked that shopkeeper exists */
...
static void addupbill(void) { /* delivers result in total */
  ...
  if (u.uinshop &&
    ...
    if (shopkeeper) {
      if (ESHK(shopkeeper)->billct) {
        ...
        setpaid();
        if ((rooms[(unsigned char)ESHK(shopkeeper)->shoproom].rtype ==
             GENERAL) == (rn2(3) == 0))
  ...
  if (roomno >= 0 && rooms[(unsigned char)roomno].rtype >= 8) {
    ...
    if (!shopkeeper) {
      ...
      if (!ESHK(shopkeeper)->following) {
        ...
        pline("Hello %s! Welcome%s to %s's %s shop!", plname,
              ESHK(shopkeeper)->visitct++ ? " again" : "", shkname(shopkeeper),
              shopnam[rooms[(unsigned char)ESHK(shopkeeper)->shoproom].rtype -
                      8]);
...
void obfree(struct obj *obj, struct obj *merge) {
  ...
  if (bp) {
    ...
    if (!bpm) {
      ...
      impossible("obfree: not on bill??", 0, 0);
      return;
    } else {
      /* this was a merger */
      bpm->bquan += bp->bquan;
...
int dopay(void) {
  ...
  if (shkp != shopkeeper && NOTANGRY(shkp)) {
    if (!ltmp) {
      ...
      if (u.ugold > ltmp) {
        pline("You give %s the %ld gold pieces he asked for.", monnam(shkp),
              ltmp);
        pay(ltmp, shkp);
      } else {
...
#define min(a, b) ((a < b) ? a : b)
      ...
      pline("you %srepay %s's expenses.",
            (u.ugold < ESHK(shkp)->robbed) ? "partially " : "", monnam(shkp));
  ...
  if (shkp != shopkeeper) {
    impossible("dopay: not to shopkeeper?", 0, 0);
    if (shopkeeper)
      setpaid();
    return (0);
  }
  ...
  pline("Thank you for shopping in %s's %s store!", shkname(shopkeeper),
...
static int dopayobj(struct bill_x *bp) {
  ...
  if (!obj) {
    impossible("Shopkeeper administration out of order.", 0, 0);
    setpaid(); /* be nice to the player */
    return (0);
  }

  if (!obj->unpaid && !bp->useup) {
    impossible("Paid object on bill??", 0, 0);
    return (1);
  }
  obj->unpaid = 0;
...
void splitbill(struct obj *obj, struct obj *otmp) {
  ...
  bp = onbill(obj);
  if (!bp) {
    impossible("splitbill: not on bill?", 0, 0);
    return;
  }
  if (bp->bquan < otmp->quan) {
    impossible("Negative quantity on bill??", 0, 0);
  }
  if (bp->bquan == otmp->quan) {
    impossible("Zero quantity on bill??", 0, 0);
  }
  bp->bquan -= otmp->quan;
...
void subfrombill(struct obj *obj) {
  ...
  if (ESHK(shopkeeper)->robbed) {
    ...
    pline("Thank you for your contribution to restock this recently plundered "
          "shop.");
    return;
  }
...
int doinvbill(int mode) /* 0: deliver count 1: paged */
{
  struct bill_x *bp;
  struct obj *obj;
  ...
  if (!shopkeeper) {
    impossible("doinvbill: no shopkeeper?", 0, 0);
    return (0);
  }
  ...
  for (bp = bill; bp - bill < ESHK(shopkeeper)->billct; bp++) {
    obj = bp_to_obj(bp);
    if (!obj) {
      impossible("Bad shopkeeper administration.", 0, 0);
      goto quit;
    }
    if (bp->useup || bp->bquan > obj->quan) {
      ...
      (void)snprintf(buf, BUFSZ, "x -  %s",
                     doname(obj)); /* MODERN: Safe sprintf replacement -
      ...
      while (cnt < 50)
        buf[cnt++] = ' ';
      (void)snprintf(
          &buf[cnt], BUFSZ - cnt, " %5ld zorkmids",
          thisused); /* MODERN: Safe sprintf replacement to prevent overflow */
      if (page_line(buf))
        goto quit;
    }
  }
  (void)snprintf(buf, BUFSZ, "Total:%50ld zorkmids",
                 totused); /* MODERN: Safe sprintf replacement - identical
...
static int realhunger(void) { /* not completely foolproof */
  int tmp = u.uhunger;
...
int shk_move(struct monst *shkp) {
  ...
  boolean uondoor = FALSE, satdoor, avoid = FALSE,
          badinv = FALSE; /* MODERN: Initialize to prevent uninitialized use */
  coord poss[9];
  int info[9];
  struct obj *ib = 0;
...
#define GDIST(x, y) ((x - gx) * (x - gx) + (y - gy) * (y - gy))
    if (Invis)
      ...
      uondoor = (u.ux == ESHK(shkp)->shd.x && u.uy == ESHK(shkp)->shd.y);
      if (uondoor) {
        if (ESHK(shkp)->billct)
