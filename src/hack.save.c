...
static void sw_i32(int fd, rh_i32_t val) {
	sw_u32(fd, (rh_u32_t)val);
}

static rh_i32_t sr_i32(int fd) {
	return (rh_i32_t)sr_u32(fd);
}
...
static void pack_flags(int fd, const struct flag *f) {
	...
	packed2 = 0;
	if (f->debug) packed2 |= 0x00000001;
	if (f->toplin & 1) packed2 |= 0x00000002;
	if (f->toplin & 2) packed2 |= 0x00000004;
	if (f->cbreak) packed2 |= 0x00000008;
	if (f->standout) packed2 |= 0x00000010;
	if (f->nonull) packed2 |= 0x00000020;
	if (f->time) packed2 |= 0x00000040;
	if (f->nonews) packed2 |= 0x00000080;
	if (f->notombstone) packed2 |= 0x00000100;
	if (f->end_own) packed2 |= 0x00000200;
	if (f->no_rest_on_space) packed2 |= 0x00000400;
	if (f->beginner) packed2 |= 0x00000800;
	if (f->female) packed2 |= 0x00001000;
	if (f->invlet_constant) packed2 |= 0x00002000;
	if (f->move) packed2 |= 0x00004000;
	if (f->mv) packed2 |= 0x00008000;
	packed2 |= (f->run & 7) << 16;
	if (f->nopick) packed2 |= 0x00080000;
	if (f->echo) packed2 |= 0x00100000;
	if (f->botl) packed2 |= 0x00200000;
	if (f->botlx) packed2 |= 0x00400000;
	if (f->nscrinh) packed2 |= 0x00800000;
	if (f->made_amulet) packed2 |= 0x01000000;
...
static void unpack_flags(int fd, struct flag *f) {
	...
	f->toplin = ((packed2 & 0x00000002) ? 1 : 0) | ((packed2 & 0x00000004) ? 2 : 0);
...
static int check_save_header(int fd, rh_hdr_t *hdr) {
	...
	if (read(fd, test_magic, 4) != 4) return 0;
	if (memcmp(test_magic, RH_MAGIC, 4) != 0) return 0;
 ...
 * 
 * WHY: Original hangup() only handled SIGHUP. Modern window managers send SIGTERM
 * when force-closing terminals, bypassing save mechanism and leaving stale locks.
 * 
...
int dosave0(int hu) {
	...
	if((fd = creat(tmpfile, FMASK)) < 0) {
		if(!hu) pline("Cannot open save file. (Continue or press Q to Quit)");
		(void) unlink(tmpfile);
		return(0);
	}
...
#ifdef HAVE_FSYNC
	if (fsync(fd) != 0) {
		if(!hu) pline("Error syncing save file.");
		(void) close(fd);
		(void) unlink(tmpfile);
		return(0);
	}
	...
	if (rename(tmpfile, SAVEF) != 0) {
		if(!hu) pline("Error finalizing save file.");
		(void) unlink(tmpfile);
		return(0);
	}
