...
/* Forward declarations for functions used but not properly visible */
  ...
  for (sx = sroom->lx; sx <= sroom->hx; sx++)
    for (sy = sroom->ly; sy <= sroom->hy; sy++) {
      ...
      if (rn2(100) < dlevel && !m_at(sx, sy) &&
        ...
        mtmp->mimic = 1;
        mtmp->mappearance = (let && rn2(10) < dlevel) ? let : ']';
        continue;
      }
...
void mkswamp(void) /* Michiel Huisjes & Fred de Wilde */
{
  struct mkroom *sroom;
  int sx, sy, i, eelct = 0;
  ...
  for (i = 0; i < 5; i++) { /* 5 tries */
    ...
    if (sroom->hx < 0 || sroom->rtype || has_upstairs(sroom) ||
        has_dnstairs(sroom))
      continue;
