...
const char *const traps[] = {" bear trap",
                             "n arrow trap",
                             " dart trap",
                             " trapdoor",
                             " teleportation trap",
...
void dotrap(struct trap *trap) {
  ...
  if (trap->tseen && !rn2(5) && ttype != PIT) {
    ...
    case PIT:
      ...
      break;
    default:
      impossible("You hit a trap of type %u", trap->ttyp, 0);
    }
  }
}
  ...
  if (!trap) {
    ...
    case TRAPDOOR:
      if (!xdnstair) {
        mtmp->mhp -= 10;
        if (in_sight)
          pline("A trap door in the ceiling opens and a rock hits %s!",
                monnam(mtmp));
        break;
      }
      if (mtmp->data->mlet != 'w') {
    ...
    case PIERC:
      break;
    default:
      impossible("Some monster encountered a strange trap.", 0, 0);
    }
  }
...
void placebc(int attach) {
  if (!uchain || !uball) {
    impossible("Where are your chain and ball??", 0, 0);
    return;
  }
